...
const handleMoveClick = (e: any) => {
  if (board.result !== RESULT_UNKNOWN) {
    const from = board.engine.getHistoryLength(); // Use engine facade
    ...
    if (from > to + 1) {
      for (let i = to + 1; i < from; i++) {
        board.engine.undoInternalMove(); // Use engine facade
      }
    } else {
...
board.onAddMove = () => {
  ...
  const counter: number = board.engine.getHistoryLength() >> 1; // Use engine facade
  const space = counter > 99 ? "    " : "   ";
  const text =
    (board.engine.sdPlayer === 0 // Use engine facade
      ? space
